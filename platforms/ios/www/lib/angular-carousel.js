...
(function() {
    ...
    .directive('rnCarousel', ['$swipe', '$window', '$document', '$parse', '$compile', '$rootScope', function($swipe, $window, $document, $parse, $compile, $rootScope) {        // internal ids to allow multiple instances
        var carouselId = 0,
            ...
            compile: function(tElement, tAttributes) {
                ...
                return function(scope, iElement, iAttributes, containerCtrl) {
                    ...
                    function autoScroll() {
                        ...
                        if (amplitude) {
                            ...
                            if (delta > rubberTreshold || delta < -rubberTreshold) {
                                ...
                                /*----------------  Added by Johanne Hansen --------------------*/
                                if(scope.$parent.animationFinished!=null){
                                    scope.$parent.$parent.animationFinished=true;       //Indicate that scroll animation is finsiehd
                                    if(scope.$parent.$parent.yesOrNo=="YES"){
                                        scope.$parent.$parent.yes();
                                        scope.$parent.$parent.yesOrNo==""
                                    }
                                    else if(scope.$parent.$parent.yesOrNo=="NO"){
                                        scope.$parent.$parent.no();
                                        scope.$parent.$parent.yesOrNo==""
                                    }
                                }
                                /*--------------------------------------------------------------*/
                    ...
                    function swipeStart(coords, event) {
                        //console.log('swipeStart', coords, event.target.tagName, event.target.className);

                        /*--------      Added by Johanne Hansen      ----------*/
                        if(event.target.tagName=="IMG"&&event.target.className=="height320 centerItem"){
                            return false;
                        }
                        else if (event.target.tagName=="DIV"&&event.target.className=="slider-slides"){
                            return false;
                        }
                        else if(event.target.tagName=="IOS-SLIDE"){
                            return false;
                        }
                        else if(coords==null||coords.x==null||coords.y==null){
                            return false;
                        }
                        else if(scope.isDataLoaded===false){
                            return false;
                        }
                        /*----------------------------------------------------------*/
                    ...
                    function swipeEnd(coords, event, forceAnimation) {
                        ...
                        /*--------      Added by Johanne Hansen      ----------*/
                        //Decide which function needs to be executed YES/NO
                        if(absMove<-minMove){
                            if(scope.$parent.$parent.no!=null&&scope.$parent.$parent.animationFinished===true){
                                scope.$parent.$parent.animationFinished=false;  //Indicate scroll animation is started
                                scope.isDataLoaded=false;
                                console.log("NO loading started");
                                scope.$parent.$parent.yesOrNo="NO";             //No function needs to be called
                            }
                        }
                        if(absMove==minMove){
                            if(scope.$parent.$parent.yes!=null&&scope.$parent.$parent.animationFinished===true){
                                scope.$parent.$parent.animationFinished=false;  //Indicate scroll animation is started
                                scope.isDataLoaded=false;
                                scope.$parent.$parent.yesOrNo="YES";            //Yes function needs to be called
                                console.log("YES loading started");
                            }
                        }
                        /*----------------------------------------------------------*/
